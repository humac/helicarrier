...
export default function AgentCard({ agent, role, status, load }: AgentCardProps) {
  const statusColor = {
    idle: "bg-zinc-500",
    active: "bg-green-500",
    ...
    >
      ...
      <div className="flex justify-between items-start mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-zinc-100">{agent}</h3>
            <p className="text-xs text-zinc-500 uppercase font-mono">{role}</p>
          </div>
        </div>
        <div className={cn("px-2 py-1 rounded text-xs font-bold uppercase tracking-wider flex items-center gap-2 border bg-zinc-950/50", 
          status === 'active' ? "border-green-500/30 text-green-400" : "border-zinc-800 text-zinc-500"
        )}>
